...
void writer_thread_func() {
  while (!finished || !write_queue.empty()) {
    ...
    while (!write_queue.empty()) {
      auto [t, Hz_frame, Ex_frame, Ey_frame] = write_queue.front();
      write_queue.pop();
      lock.unlock();
      ...
      int Nx = static_cast<int>(sqrt(Ex_frame.size()));
      int Ny = Nx;
...
// --- FDTD main loop ---
  ...
  // --- Update Ex ---
    ...
    if(t % data_capture_interval == 0) {
      ...
      write_queue.push({t, std::move(Hz_copy), std::move(Ex_copy), std::move(Ey_copy)});
      cv.notify_one();
    }
  }
