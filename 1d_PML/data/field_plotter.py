...
# Load config
with open("../test/control_file.scf", "r") as file:
    json_data = json.load(file)

iterations = json_data["iterations"]
interval = json_data["data_capture_interval"]
domain_min = json_data["domain_x"][0]
domain_max = json_data["domain_x"][1]
...
def process_frame(i):
    ...
    x_axis = np.linspace(domain_min, domain_max, len(data))
