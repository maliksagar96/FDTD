...
int main() {
  ...
  double pml_cond_e = -(m+1)*log(1e-9) * c0 * EPSILON_0 / (2 * refractive_index[N_space_cells-1 - pml_size] * pml_size * step_size);
  
  // Right PML
for (int i = 0; i < pml_size; i++) {
    double x = static_cast<double>(i) / pml_size;  // goes 0 → 1
    sigma[N_space_cells - pml_size + i] = pml_cond_e * pow(x, m);
    sigma_h[N_space_cells - pml_size + i] =
        (pml_cond_e * MU_0 / (EPSILON_0 * eps[N_space_cells - pml_size + i])) *
        pow((i + 0.5) / pml_size, m);
}

//left PML
for (int i = 0; i < pml_size; i++) {
    double x = static_cast<double>(pml_size - i) / pml_size;   // goes 1 → 1/pml_size
    sigma[i] = pml_cond_e * pow(x, m);
    sigma_h[i] = (pml_cond_e * MU_0 / (EPSILON_0 * eps[i])) *
                 pow((pml_size - i - 0.5) / pml_size, m);
}
...
for (int i = 0; i < N_space_cells; i++) {
    sigma_out << sigma[i] << std::endl;
    sigma_h_out << sigma_h[i] << std::endl;
}
